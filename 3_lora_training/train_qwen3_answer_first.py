#!/usr/bin/env python3
"""
Qwen3-4B 训练脚本 - 答案优先格式

关键改动：
1. sentiment 定位使用 find() 而非 rfind()（答案在前）
2. 支持软标签蒸馏
3. 适配 Qwen3-4B 模型（ChatML 格式）

Usage:
    # 测试模式
    python train_qwen3_answer_first.py --data data/train_answer_first.json --test

    # 全量训练
    python train_qwen3_answer_first.py --data data/train_answer_first.json --epochs 3
"""
...
DEFAULT_OUTPUT = "models/qwen3-4b-answer-first"
...
class AnswerFirstTrainer:
    ...
    def _pre_tokenize_dataset(self, raw_data):
        ...
        for item in raw_list:
            ...
            # 关键改动：使用 find() 而非 rfind()（答案在开头）
            sentiment_pos = -1
            target_str = '"sentiment": '
            first_idx = full_text.find(target_str)  # ← 改为 find
            if first_idx != -1:
