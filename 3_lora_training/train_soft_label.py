...
DEFAULT_OUTPUT = "models/qwen3-4b-sentiment-soft"
MAX_SEQ_LENGTH = 512
LORA_RANK = 16
RANDOM_STATE = 3407
...
class SoftLabelTrainer:
    ...
    def _pre_tokenize_dataset(self, raw_data):
        ...
        - 这个数量就是 sentiment 值 token 的绝对索引（即 0/1/2 所在位置）
        - 绕过了 BPE 上下文依赖问题（Token 窗口搜索无法处理合并 token）
        """
        records = []
        ...
        for item in raw_list:
            ...
            # rfind：取最后一次出现（assistant 输出中的，而非 system prompt 中的）
            sentiment_pos = -1
